...
function Signin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://eduxcel-api-13march.onrender.com/api/signin', formData);
      console.log('Signin success');
      const token = response.data.token;

      localStorage.setItem('token', token);

      const profileResponse = await axios.get('https://eduxcel-api-13march.onrender.com/api/profile', {
        headers: {
