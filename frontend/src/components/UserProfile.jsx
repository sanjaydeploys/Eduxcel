...
useEffect(() => {
  const fetchUserProfile = async () => {
    try {
      ...
      const response = await fetch('https://eduxcel-api-13march.onrender.com/api/profile', {
        method: 'GET',
        headers: {
 ...
 const handleUpdateProfile = async (updatedProfileData) => {
  ...
  const confirmLogout = async () => {
    try {
      ...
      await axios.post('https://eduxcel-api-13march.onrender.com/api/logout');
 ...
 <meta name="author" content="EduXcel" />        <script type="application/ld+json">
  ...
  <div className="profile-info">
    <div className="profile-image-container">
      <motion.img
        src={`https://eduxcel-api-13march.onrender.com/${userProfile.profileImage}?key=${Date.now()}`}
        alt="Profile"
